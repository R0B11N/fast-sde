...
use fast_sde::mc::mc_engine::{
    ...
    mc_price_option_gbm, mc_rho_european_call_gbm_pathwise, mc_vega_european_call_gbm_pathwise,
    GreeksConfig, McConfig,
};
...
fn run_canonical_benchmark() {
    ...
    println!(
        "Rust benchmark results written to {}",
...
fn run_demo_mode() {
    ...
    let mut timer = Timer::new();
    timer.start();
    let (mc_price_european, _variance_european) =
        mc_price_option_gbm(&cfg_european_call).expect("Valid configuration");
    let price_time_european = timer.elapsed_ms();
    println!(
        "MC Price (European Call): {} ({} ms)",
        mc_price_european, price_time_european
    );
    let analytic_price_european = bs_analytic::bs_call_price(
        cfg_european_call.s0,
        k,
        cfg_european_call.r,
        cfg_european_call.sigma,
        cfg_european_call.t,
    );
    println!(
        "Analytic Price (European Call): {}",
        analytic_price_european
    );
    ...
    println!(
        "MC Delta (European Call Pathwise): {} ({} ms)",
        mc_delta_european, delta_time_european
    );
    let analytic_delta_european = bs_analytic::bs_call_delta(
        cfg_european_call.s0,
        k,
        cfg_european_call.r,
        cfg_european_call.sigma,
    ...
    println!(
        "Analytic Delta (European Call): {}",
        analytic_delta_european
    );
    ...
    println!(
        "MC Vega (European Call Pathwise): {} ({} ms)",
        mc_vega_european, vega_time_european
    );
    let analytic_vega_european = bs_analytic::bs_call_vega(
        cfg_european_call.s0,
        k,
        cfg_european_call.r,
        cfg_european_call.sigma,
    ...
    println!(
        "MC Rho (European Call Pathwise): {} ({} ms)",
        mc_rho_european, rho_time_european
    );
    let analytic_rho_european = bs_analytic::bs_call_rho(
        cfg_european_call.s0,
        k,
        cfg_european_call.r,
        cfg_european_call.sigma,
    ...
    println!(
        "MC Gamma (European Call Finite Diff): {} ({} ms)",
        mc_gamma_european, gamma_time_european
    );
    let analytic_gamma_european = bs_analytic::bs_call_gamma(
        cfg_european_call.s0,
        k,
        cfg_european_call.r,
        cfg_european_call.sigma,
    ...
    println!(
        "Analytic Gamma (European Call): {}",
        analytic_gamma_european
    );
    ...
    timer_asian.start();
    let (mc_price_asian, _variance_asian) =
        mc_price_option_gbm(&cfg_asian_call).expect("Valid configuration");
    let price_time_asian = timer_asian.elapsed_ms();
    println!(
        "MC Price (Asian Call): {} ({} ms)\n",
    ...
    let (mc_price_barrier_call, _variance_barrier_call) =
        mc_price_option_gbm(&cfg_barrier_call_up_and_out).expect("Valid configuration");
    let price_time_barrier_call = timer_barrier_call.elapsed_ms();
    println!(
        "MC Price (Barrier Call Up and Out): {} ({} ms)\n",
    ...
    let (mc_price_barrier_put, _variance_barrier_put) =
        mc_price_option_gbm(&cfg_barrier_put_up_and_out).expect("Valid configuration");
    let price_time_barrier_put = timer_barrier_put.elapsed_ms();
    println!(
        "MC Price (Barrier Put Up and Out): {} ({} ms)\n",
    ...
    let path_data_for_csv: Vec<(f64, f64, f64)> = (0..paths)
        ...
        .map(|i| {
            ...
            let delta_path_european = if *path_prices.last().unwrap() > k {
                *path_prices.last().unwrap() / s0
            } else {
                0.0
            };
            (
                *path_prices.last().unwrap(),
                payoff_european,
                delta_path_european,
            )
        })
