...
impl HestonCalibrator {
    ...
    fn mc_price(&self, params: &HestonParams, strike: f64, time_to_expiry: f64, paths: usize) -> f64 {
    ...
    pub fn calibrate_simple(&self) -> HestonParams {
        ...
        let total_combinations = kappa_range.len() * theta_range.len() * xi_range.len() * rho_range.len();
            ...
            for &theta in &theta_range {
                for &xi in &xi_range {
                    for &rho in &rho_range {
                        ...
                        if combination % 20 == 0 {
                            println!("Progress: {}/{} combinations, best error: {:.6}", 
...
fn main() {
    ...
    for (i, point) in market_data.iter().enumerate() {
        println!("  {}: K={}, T={}, σ_impl={:.3}, Price={:.4}", 
                 i + 1, point.strike, point.time_to_expiry, 
                 point.implied_vol, point.market_price);
    }
    println!();
    ...
    for (i, point) in calibrator.market_data.iter().enumerate() {
        let model_price = calibrator.mc_price(&calibrated_params, point.strike, point.time_to_expiry, 50_000);
        let error = ((model_price - point.market_price) / point.market_price * 100.0).abs();
        
        println!("  Point {}: Market={:.4}, Model={:.4}, Error={:.2}%", 
                 i + 1, point.market_price, model_price, error);
    }
