...
use fast_sde::mc::mc_engine::{mc_price_option_gbm, mc_delta_european_call_gbm_pathwise, 
                               mc_gamma_european_call_gbm_finite_diff_batched, McConfig, GreeksConfig};
            ...
            Command::new("wmic")
                ...
                .and_then(|output| {
                    String::from_utf8_lossy(&output.stdout)
                        ...
                        .and_then(|line| line.trim_start_matches("TotalPhysicalMemory=").trim().parse::<u64>().ok())
...
fn run_heston_benchmarks() -> Vec<BenchmarkResult> {
    ...
    for (scheme, scheme_name) in &schemes {
        ...
        for i in 0..paths {
            ...
            for _ in 0..steps {
                heston.step(&mut s, &mut v, dt, &mut rng).expect("Step should succeed");
            }
...
fn write_results_to_csv(results: &[BenchmarkResult], system_info: &SystemInfo, filename: &str) {
    ...
    writeln!(file, "# Benchmark Date: {}", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC")).unwrap();
    writeln!(file, "#").unwrap();
    
    // Write CSV header
    writeln!(file, "Benchmark,Paths,Time_ms,Throughput_paths_per_sec,Value,Analytic_Value,Relative_Error").unwrap();
    ...
    for result in results {
        writeln!(file, "{},{},{:.2},{:.0},{:.6},{},{}", 
                 ...
                 result.value,
                 result.analytic_value.map(|v| format!("{:.6}", v)).unwrap_or_else(|| "N/A".to_string()),
                 result.relative_error.map(|e| format!("{:.6}", e)).unwrap_or_else(|| "N/A".to_string())
        ).unwrap();
    }
...
fn main() {
    ...
    println!("{:=<80}", "");
    println!("{:<35} {:>8} {:>12} {:>15} {:>10} {:>10} {:>12}", 
             "Benchmark", "Paths", "Time (ms)", "Throughput", "Value", "Analytic", "Rel Error");
    println!("{:-<80}", "");
    
    for result in &all_results {
        println!("{:<35} {:>8} {:>12.2} {:>15.0} {:>10.4} {:>10} {:>12}", 
                 result.name,
                 result.paths,
                 result.time_ms,
                 result.throughput_paths_per_sec,
                 result.value,
                 result.analytic_value.map(|v| format!("{:.4}", v)).unwrap_or_else(|| "N/A".to_string()),
                 result.relative_error.map(|e| format!("{:.2}%", e * 100.0)).unwrap_or_else(|| "N/A".to_string())
        );
    }
    ...
    println!("4. Ensure {} CPU threads available", system_info.rayon_threads);
}
