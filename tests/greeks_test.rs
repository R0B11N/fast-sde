...
use fast_sde::mc::mc_engine::{
    mc_delta_european_call_gbm_pathwise, mc_gamma_european_call_gbm_finite_diff,
    mc_gamma_european_call_gbm_finite_diff_batched, mc_rho_european_call_gbm_pathwise,
    mc_vega_european_call_gbm_pathwise, GreeksConfig, McConfig,
};
...
fn test_mc_delta_pathwise_vs_analytic() {
    ...
    assert!(
        rel_error < 0.01,
        "Relative error for Delta exceeds 1%: {}",
        rel_error
    );
}
...
fn test_bs_call_theta_analytic() {
    ...
    assert!(
        rel_error < 1e-7,
        "Relative error for Theta exceeds tolerance: {}",
        rel_error
    );
}
...
fn test_mc_rho_pathwise_vs_analytic() {
    ...
    assert!(
        rel_error < 0.02,
        "Relative error for Rho exceeds 2%: {}",
        rel_error
    );
}
...
fn test_mc_gamma_finite_diff_vs_analytic() {
    ...
    println!(
        "Relative Error (Batched): {:.4}%",
        rel_error_batched * 100.0
    );

    // Using a looser tolerance for finite difference methods
    assert!(
        rel_error < 0.05,
        "Relative error for Gamma (simple) exceeds 5%: {}",
        rel_error
    );
    assert!(
        rel_error_batched < 0.05,
        "Relative error for Gamma (batched) exceeds 5%: {}",
        rel_error_batched
    );
}
...
fn test_mc_vega_rho_comprehensive_ci() {
    ...
    let vega_std = (vega_results
        .iter()
        .map(|x| (x - vega_mean).powi(2))
        .sum::<f64>()
        / (n_runs - 1) as f64)
        .sqrt();
    ...
    let rho_std = (rho_results
        .iter()
        .map(|x| (x - rho_mean).powi(2))
        .sum::<f64>()
        / (n_runs - 1) as f64)
        .sqrt();
    ...
    println!(
        "\n=== Comprehensive MC Greeks Test Results ({} runs, {} paths) ===",
        n_runs, n_paths
    );
    println!("Vega:");
    ...
    println!(
        "  Relative Error: {:.4}%",
        (vega_mean - analytic_vega).abs() / analytic_vega * 100.0
    );
    ...
    println!(
        "  Relative Error: {:.4}%",
        (rho_mean - analytic_rho).abs() / analytic_rho * 100.0
    );

    // Assert that analytical values fall within 95% CI
    assert!(
        analytic_vega >= vega_ci_95_lo && analytic_vega <= vega_ci_95_hi,
        "Analytic Vega {} not in 95% CI [{}, {}]",
        analytic_vega,
        vega_ci_95_lo,
        vega_ci_95_hi
    );
    assert!(
        analytic_rho >= rho_ci_95_lo && analytic_rho <= rho_ci_95_hi,
        "Analytic Rho {} not in 95% CI [{}, {}]",
        analytic_rho,
        rho_ci_95_lo,
        rho_ci_95_hi
    );
