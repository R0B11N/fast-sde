...
fn test_bs_mc_vs_analytic() {
    ...
    let (mc_price_with_cv, variance_with_cv) = mc_price_option_gbm(&cfg_with_cv).expect("Valid configuration");
    ...
    let (mc_price_without_cv, variance_without_cv) = mc_price_option_gbm(&cfg_without_cv).expect("Valid configuration");
    ...
    assert!(vrf > 1.2, "Variance Reduction Factor ({}) is not greater than 1.2", vrf);
}
...
fn test_asian_option_cv() {
    ...
    let (_, variance_without_cv) = mc_price_option_gbm(&cfg_without_cv).expect("Valid configuration");
    ...
    assert!(vrf > 1.1, "Variance Reduction Factor for Asian Call ({}) is not greater than 1.1", vrf);
}
