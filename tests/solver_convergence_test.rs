...
fn gbm_exact_solution_path(s0: f64, r: f64, sigma: f64, _t_end: f64, dt: f64, normal_draws: &[f64]) -> Vec<f64> {
...
fn test_euler_maruyama_ou_convergence() {
    ...
    for i in 0..(errors.len() - 1) {
        assert!(errors[i] > errors[i+1], "Euler-Maruyama did not converge (weak) as expected at step {}", i);
    }
    // Assert that the final absolute error is below a threshold for weak convergence
    assert!(*errors.last().unwrap() < 0.15, "Euler-Maruyama final absolute error ({}) is too high for weak convergence", errors.last().unwrap());
}
...
fn test_euler_maruyama_gbm_strong_convergence() {
    ...
    for num_steps in &[10, 20, 40, 80, 160] {
        ...
        for i in 0..num_paths {
            ...
            let exact_path = gbm_exact_solution_path(s0, gbm_process.mu, gbm_process.sigma, t_end, dt, &normal_draws);
    ...
    println!("\nEuler-Maruyama GBM Strong Convergence RMSEs: {:?}", rms_errors);
    ...
    for i in 0..(rms_errors.len() - 1) {
        ...
        assert!(ratio > 1.2 && ratio < 1.6, "Strong convergence ratio not as expected at step {}: {}", i, ratio);
    }
    assert!(*rms_errors.last().unwrap() < 1.0, "Euler-Maruyama final RMSE ({}) is too high for strong convergence", rms_errors.last().unwrap());
}
...
fn test_milstein_ou_convergence() {
    ...
    for i in 0..(errors.len() - 1) {
        assert!(errors[i] > errors[i+1], "Milstein did not converge (weak) as expected at step {}", i);
    }
    // Assert that the final absolute error is below a threshold for weak convergence
    assert!(*errors.last().unwrap() < 0.1, "Milstein final absolute error ({}) is too high for weak convergence", errors.last().unwrap());
}
...
fn test_srk_ou_convergence() {
    ...
    // Assert weak convergence
    for i in 0..(errors.len() - 1) {
        assert!(errors[i] > errors[i+1], "SRK did not converge (weak) as expected at step {}", i);
    }
    assert!(*errors.last().unwrap() < 0.05, "SRK final absolute error ({}) is too high for weak convergence", errors.last().unwrap());
}
