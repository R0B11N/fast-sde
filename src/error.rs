...
impl fmt::Display for SdeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            SdeError::InvalidParameters { parameter, value, constraint } => {
                write!(f, "Invalid parameter '{}' = {}: {}", parameter, value, constraint)
            }
            SdeError::NumericalInstability { method, reason } => {
                write!(f, "Numerical instability in {}: {}", method, reason)
            }
            SdeError::FellerConditionViolation { kappa, theta, xi, feller_value } => {
                write!(f, 
            ...
            SdeError::MonteCarloError { paths, reason } => {
                write!(f, "Monte Carlo simulation error with {} paths: {}", paths, reason)
            }
            SdeError::PayoffError { payoff_type, reason } => {
                write!(f, "Payoff calculation error for {}: {}", payoff_type, reason)
            }
            SdeError::RandomGenerationError { reason } => {
                write!(f, "Random number generation error: {}", reason)
            }
            SdeError::CalibrationError { reason, current_error } => {
                match current_error {
                    Some(err) => write!(f, "Calibration failed (current error: {:.6}): {}", err, reason),
                    None => write!(f, "Calibration failed: {}", reason),
                }
            }
            SdeError::UnsupportedOperation { operation, context } => {
                write!(f, "Unsupported operation '{}' in context: {}", operation, context)
            }
        }
    }
}
