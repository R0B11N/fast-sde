...
impl Heston {
    ...
    pub fn new_with_scheme_quiet(params: HestonParams, scheme: HestonScheme, suppress_warnings: bool) -> SdeResult<Self> {
    ...
    fn validate_params(params: &HestonParams) -> SdeResult<()> {
        ...
        if params.kappa > 100.0 {
            return Err(SdeError::InvalidParameters {
                ...
                value: params.kappa,
                constraint: "extremely high mean reversion speed (>100) may cause numerical issues".to_string(),
            });
        }
    ...
    pub fn step<R: Rng + ?Sized>(&self, s: &mut f64, v: &mut f64, dt: f64, rng: &mut R) -> SdeResult<()> {
    ...
    fn step_full_truncation_euler(&self, s: &mut f64, v: &mut f64, dt: f64, dw_s: f64, dw_v: f64) -> SdeResult<()> {
        let sqrt_dt = dt.sqrt();
    ...
    fn step_andersen_qe<R: Rng + ?Sized>(&self, s: &mut f64, v: &mut f64, dt: f64, dw_s: f64, dw_v: f64, rng: &mut R) -> SdeResult<()> {
        let _sqrt_dt = dt.sqrt();
        
        // QE scheme for variance
        let m = self.params.theta + (*v - self.params.theta) * (-self.params.kappa * dt).exp();
        let s2 = *v * self.params.xi * self.params.xi * (-self.params.kappa * dt).exp() / self.params.kappa * 
        ...
        let ds_over_s = self.params.r * dt + k0 + k1 * *v + k2 * v_next + 
                        v.max(0.0).sqrt() * k3.sqrt() * dw_s;
    ...
    fn step_alfonsi(&self, s: &mut f64, v: &mut f64, dt: f64, dw_s: f64, dw_v: f64) -> SdeResult<()> {
        let sqrt_dt = dt.sqrt();
        ...
        let correction = if *v > 0.0 && v_aux > 0.0 {
            gamma * xi * xi * dt * (1.0 / v_aux_pos.sqrt() - 1.0 / v.sqrt()) * 
            (dw_v * dw_v - dt) / (2.0 * sqrt_dt)
        } else {
            0.0
        };
        ...
        *v = (v_aux + correction).max(0.0);
        ...
        // Update stock price
        let sqrt_v_avg = if *v > 0.0 { (v.sqrt() + (*v).sqrt()) / 2.0 } else { 0.0 };
...
mod tests {
    ...
    fn test_heston_schemes() {
        ...
        for scheme in &schemes {
            ...
            // Run a few steps
            for _ in 0..100 {
                heston.step(&mut s, &mut v, 0.01, &mut rng).expect("Step should succeed");
                assert!(s > 0.0, "Stock price must remain positive");
                assert!(v >= 0.0, "Variance must be non-negative");
            }
            
            println!("Scheme {}: S_T = {:.4}, V_T = {:.6}", 
                     heston.scheme_name(), s, v);
        }
    }
